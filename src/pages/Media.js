  ...
  return (
    <>
          <div className='flex flex-col lg:flex-row pt-20 bg-Owhite w-full'>
              ...
              <img className="h-[90vh] hidden lg:block" src={poster_path ? `${imgUrl}${poster_path}` : FallbackPoster} alt={title || name} />
            ...
            <div className='ml-6 pt-5 flex flex-col justify-between overflow-x-hidden'>
              <div className=''>
                ...
                <h2 className="ZT-Italic text-Oblue text-md lg:text-3xl py-2 lg:py-4">
                  {media_type === "movie" ? "Movie" : "TV Show"} {year && `(${year})`}
                </h2>
                {genres &&
                  genres.map(({ name, id }) => (
                    <Link to={`/genre/${media_type}/${id}/${name}`} key={name}>
                      <button className='bg-Oblue text-Owhite rounded-full text-xs lg:text-lg px-4 py-1 mr-2 mb-4 lg:mb-6 ACondensed'>{name}</button>
                    </Link>
                  ))}
              
              <div>
                <p className='text-Oblue text-2xl lg:text-4xl ZT font-semibold'>overview</p>
                <p className='text-Oblue text-sm lg:text-2xl ACondensed mr-12 pt-2 pb-4 lg:pb-0 min-h-36 lg:min-h-0'>{overview}</p>
              </div>
              </div>
